...
exports.encodeSystemGrantHint = encodeSystemGrantHint
exports.decodeSystemGrantHint = decodeSystemGrantHint
...
exports.encodeSystemPending = encodeSystemPending
exports.decodeSystemPending = decodeSystemPending
...
function encodeSystemPendingKey(key) {
...
function encodeSystemPending(entry) {
  const state = { start: 0, end: 0, buffer: null }
  c.uint.preencode(state, entry.weight)
  c.uint.preencode(state, entry.anchored)
  c.uint.preencode(state, entry.flushes)
  state.buffer = b4a.alloc(state.end)
  c.uint.encode(state, entry.weight)
  c.uint.encode(state, entry.anchored)
  c.uint.encode(state, entry.flushes)
  return state.buffer
}

function decodeSystemPending(value) {
  const state = { start: 0, end: value.byteLength, buffer: value }
  return {
    weight: c.uint.decode(state),
    anchored: c.uint.decode(state),
    flushes: c.uint.decode(state)
  }
}

function encodeSystemGrantKey(key) {
  const buf = b4a.allocUnsafe(33)
  buf[0] = 4
  buf.set(key, 1)
  return buf
...
function encodeSystemGrantHint(hint) {
  const state = { start: 0, end: 0, buffer: null }
  c.fixed32.preencode(state, hint.key)
  c.uint.preencode(state, hint.length)
  c.uint.preencode(state, hint.weight)
  state.buffer = b4a.alloc(state.end)
  c.fixed32.encode(state, hint.key)
  c.uint.encode(state, hint.length)
  c.uint.encode(state, hint.weight)
  return state.buffer
}

function decodeSystemGrantHint(value) {
  const state = { start: 0, end: value.byteLength, buffer: value }
  return {
