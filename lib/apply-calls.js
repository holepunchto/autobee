...
class ApplyCalls {
  ...
  async createAnchor() {
    const last = this.applying[this.applying.length - 1]
    // just to unblock, wrap in static anchor in general like in autobase
    return { key: last.key, length: last.length }
  }
}
