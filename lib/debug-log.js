...
// only a bare {key, length} pair collapses to id@length - a richer object
// that happens to carry both must still print all of its fields
function isHead(v) {
  if (typeof v.length !== 'number') return false
  if (!b4a.isBuffer(v.key) && v.key !== null) return false
  return Object.keys(v).length === 2
}

// a stalled writer is only explicable next to what our peers claim to hold
function coreState(core) {
  if (!core) return String(core)

  const peers = []
  for (const p of core.peers) {
    peers.push(p.remoteLength + '/' + p.remoteContiguousLength)
  }

  return {
    length: core.length,
    contiguous: core.contiguousLength,
    remoteContiguous: core.remoteContiguousLength,
    peers
  }
}
