...
module.exports = class Systembee {
  ...
  async addWriter(key, { length = 0, weight = 1, isGenesis = false } = {}) {
    ...
    if (weight <= current) {
      ...
      this.update(key, length, -1, -1, false, true, false, false, false, null)
      return
    }

    this.update(key, length, -1, weight, isGenesis, true, false, false, false, null)
  }
  ...
  async ackWriter(key, { length = 0 } = {}) {
    ...
    this.update(key, length, -1, -1, false, false, false, true, false, null)
  }
  ...
  async removeWriter(key, { length = 0 } = {}) {
    ...
    this.update(key, length, -1, 0, false, false, true, false, false, null)
  }

  addAnchor(key, { length = 1 } = {}) {
    this.update(key, length, -1, -1, false, true, false, false, true, null)
  }
  ...
  addNode(node) {
    ...
    this.update(node.key, node.length, node.weight, -1, false, false, false, false, false, node.timestamp)
  }
  ...
  update(key, length, weight, maxWeight, isGenesis, isAdded, isRemoved, isAcked, isAnchor, timestamp) {
    const id = b4a.toString(key, 'hex')
    ...
    // only batch[0]
    if (timestamp && upd.timestamp === null) {
  ...
  async _updateWriter(upd, oplog) {
    ...
    const record = {
      ...
      timestamp: isOplog ? upd.timestamp : null
    }
    ...
    return [
      ...
      null,
      record.isRemoved ? encoding.encodeSystemIndexerKey(upd.key) : null
    ]
  }
