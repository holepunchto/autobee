...
module.exports = class Systembee {
  ...
  async addWriter(key, { length = 0, weight = 1, referrer = null } = {}) {
    ...
    if (weight <= current) {
      ...
      this.update(key, length, -1, -1, null, true, false, false)
      return
    }

    this.update(key, length, -1, weight, referrer, true, false, false)
  }
  ...
  async ackWriter(key, { length = 0 } = {}) {
    ...
    this.update(key, length, -1, -1, null, false, false, true)
  }
  ...
  async removeWriter(key, { length = 0 } = {}) {
    ...
    this.update(key, length, -1, 0, null, false, true, false)
  }
  ...
  addNode(node) {
    ...
    this.update(node.key, node.length, node.weight, -1, null, false, false, false)
  }
  ...
  update(key, length, weight, maxWeight, referrer, isAdded, isRemoved, isAcked) {
    const id = b4a.toString(key, 'hex')
  ...
  async _updateWriter(upd, oplog) {
    ...
    const record = {
      ...
      weight: upd.weight !== -1 ? upd.weight : v ? v.weight : 0,
      maxWeight: upd.maxWeight !== -1 ? upd.maxWeight : v ? v.maxWeight : 0,
