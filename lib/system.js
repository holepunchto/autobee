...
module.exports = class Systembee {
  ...
  async demoteWriter(key, { length = 0, weight = 0, coord = null } = {}) {
    const info = await this.get(key, { unflushed: true })
    if (length === 0) {
      length = info ? info.length : 0
    }

    const grant = coord ? { key: coord.key, length: coord.length, weight } : null

    this.update(key, length, -1, weight, false, true, false, false, false, -1, grant)
  }
  ...
  async flush(batch, bee) {
    ...
    const pendings = await Promise.all(updates.map((upd) => this._updatePending(upd)))
  ...
  async _updateGrants(upd) {
    ...
    const grants = node ? encoding.decodeSystemGrants(node.value).grants : []
    ...
    return { key, value: encoding.encodeSystemGrants({ grants }) }
  }

  async _updatePending(upd) {
    if (upd.pending === -1 && upd.maxWeight === -1) return null

    const key = encoding.encodeSystemPendingKey(upd.key)
    const node = await this.bee.get(key)
    const existing = node ? encoding.decodeSystemPending(node.value) : 0

    const info = await this.get(upd.key, { unflushed: true })
    const max = info ? info.maxWeight : 0

    const want = Math.max(existing, upd.pending)
    if (want > max) {
      if (want === existing) return null
      return { key, value: encoding.encodeSystemPending(want) }
    }

    return node ? { key, remove: true } : null
  }

  async pendingPromotion(key) {
    const node = await this.bee.get(encoding.encodeSystemPendingKey(key))
    return node ? encoding.decodeSystemPending(node.value) : 0
  }

  async *listPendingPromotions() {
    const gte = b4a.from([5])
    const lt = b4a.from([6])
    for await (const data of this.bee.createReadStream({ gte, lt })) {
      yield { key: data.key.subarray(1), weight: encoding.decodeSystemPending(data.value) }
    }
  }

  async grants(key) {
    const node = await this.bee.get(encoding.encodeSystemGrantKey(key))
    return node ? encoding.decodeSystemGrants(node.value).grants : []
  }
  ...
  async grantedWeight(key, link) {
    ...
    for (const g of await this.grants(key)) {
      if (g.length !== link.length) continue
      if (!b4a.equals(g.key, link.key)) continue
      if (g.weight > best) best = g.weight
    }
