...
module.exports = class Systembee {
  ...
  async addWriter(key, { length = 0, weight = 1, isGenesis = false, coord = null } = {}) {
    ...
    if (weight <= current) {
      if (info && !info.isRemoved && length <= info.length && !grant) return

      this.update(key, length, -1, -1, false, true, false, false, false, -1, grant)
      return
    }

    const w = isGenesis ? weight : -1
    this.update(key, length, w, weight, isGenesis, true, false, false, false, -1, grant)
  }
