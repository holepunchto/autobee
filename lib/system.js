...
module.exports = class Systembee {
  ...
  async addWriter(
    ...
    if (effective <= current) {
      ...
      this.update(key, length, -1, -1, false, true, false, false, -1, hint, pending)
      return
    }

    const w = isGenesis ? effective : -1
    this.update(key, length, w, effective, isGenesis, true, false, false, -1, hint, pending)
  }
  ...
  async ackWriter(key, { length = 0 } = {}) {
    ...
    this.update(key, length, -1, -1, false, false, false, true, -1, null)
  }
  ...
  async removeWriter(key, { length = 0 } = {}) {
    ...
    this.update(key, length, -1, 0, false, false, true, false, -1, null)
  }

  addAnchor(key) {
    this.update(key, 0, -1, -1, false, true, false, false, -1, null)
  }
