...
module.exports = class Systembee {
  ...
  async addWriter(
    ...
    const pending = coord && !isGenesis && effective > 0 ? weight : -1

    const current = info ? info.maxWeight : 0
    if (effective <= current) {
      if (info && !info.isRemoved && length <= info.length && !hint && pending === -1) return
  ...
  async reset({ timeout } = {}) {
    ...
    this.pendingDigest = info && info.pending ? info.pending : []
    this.updates.clear()
    this.writers.clear()
    this._pendingIndex = null
  ...
  async flush(batch, bee) {
    ...
    const pendings = await this._flushPending(updates, this.flushes + 1)
  ...
  async _flushUpdate(upd, oplog) {
    const [writer, grant] = await Promise.all([
      ...
      this._updateGrantHint(upd)
    ])

    return { writer, grant }
  }

  async _updateGrantHint(upd) {
  ...
  async _flushPending(updates, flushes) {
    let relevant = false
    ...
    const index = await this._loadPendingIndex()
    if (!index.size && !updates.some((u) => u.pending !== -1)) return null
    ...
    for (const upd of updates) {
      ...
      const existing = index.get(hex) || null
      ...
      const node = await this.bee.get(encoding.encodeSystemGrantKey(upd.key))
      if (node) {
        const hint = encoding.decodeSystemGrantHint(node.value)
        if (hint.weight > anchored) anchored = hint.weight
      }
    ...
    for (const w of weights) {
      while (this.pendingDigest.length < w) this.pendingDigest.push(0)
      this.pendingDigest[w - 1] = flushes
    }

    return ops
  }

  async _loadPendingIndex({ activeRequests } = {}) {
    if (this._pendingIndex !== null) return this._pendingIndex
  ...
  async pendingPromotion(key, { activeRequests } = {}) {
    if (this._pendingIndex !== null) {
      const entry = this._pendingIndex.get(b4a.toString(key, 'hex'))
  ...
  async *listPendingPromotions({ activeRequests } = {}) {
    const index = await this._loadPendingIndex({ activeRequests })
    for (const [hex, entry] of index) {
      yield { key: b4a.from(hex, 'hex'), weight: entry.weight, flushes: entry.flushes }
    }
  }

  async grantHint(key, { activeRequests } = {}) {
