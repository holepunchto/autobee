...
module.exports = class Systembee {
  ...
  // concurrent races converge and honest claims never decrease
  async addWriter(key, { length = 0, weight = 1, isGenesis = false } = {}) {
    ...
    if (weight <= current) {
      ...
      this.update(key, length, -1, -1, false, true, false, false, false)
      return
    }

    this.update(key, length, -1, weight, isGenesis, true, false, false, false)
  }
  ...
  async ackWriter(key, { length = 0 } = {}) {
    ...
    this.update(key, length, -1, -1, false, false, false, true, false)
  }
  ...
  async removeWriter(key, { length = 0 } = {}) {
    ...
    this.update(key, length, -1, 0, false, false, true, false, false)
  }

  addAnchor(key, { length = 1 } = {}) {
    this.update(key, length, -1, -1, false, true, false, false, true)
  }
  ...
  addNode(node) {
    ...
    this.update(node.key, node.length, node.weight, -1, false, false, false, false, false)
  }
  ...
  update(key, length, weight, maxWeight, isGenesis, isAdded, isRemoved, isAcked, isAnchor) {
    const id = b4a.toString(key, 'hex')
