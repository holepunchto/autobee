...
module.exports = class Systembee {
  ...
  async reset() {
    const node = await this.checkout.get(INFO_KEY)
    if (!node) return
  ...
  async prepare(batch, bee, debug) {
    ...
    if (t.undo) {
      // yolo for now
      const co = this.bee.checkout({
        key: t.undo.key,
        length: t.undo.length,
        writable: true
      })
      if (this.bee !== this.checkout) await this.checkout.close()
      this.checkout = co
      const h = this.heads
      await this.reset()
    }
  ...
  addNode(node, debug) {
    ...
    for (const h of this.heads) {
      if (h.key.equals(node.key)) {
        console.log(debug, 'NO', this.heads, node)
        process.exit(42)
      }
    }
  ...
  async get(key) {
    const node = await this.checkout.get(encodeWriterKey(key))
    if (node === null) return null
    return encoding.decodeSystemWriter(node.value)
  }

  async *list() {
    for await (const data of this.checkout.createReadStream()) {
      if (data.key[0] === 1) {
  ...
  async _updateWriter(upd, oplog) {
    ...
    if (upd.isAcked || !changed || upd.length === 0) {
      const node = await this.checkout.get(k)
  ...
  async flush(batch, bee, debug) {
    const node = batch[batch.length - 1] // yolo

    this.addNode(node, debug)

    const promises = []
    for (const upd of this.updates.values()) {
      promises.push(this._updateWriter(upd, node.key))
    }

    const w = this.checkout.write()
