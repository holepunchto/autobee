...
module.exports = class Systembee {
  ...
  async _flushPending(updates) {
    ...
    for (const upd of updates) {
      ...
      const existing = index.get(hex) || 0
      ...
      if (want > anchored) {
        ...
        ops.push({
          ...
          value: encoding.encodeSystemPending(want)
        })
  ...
  async pendingPromotion(key, { activeRequests } = {}) {
    if (this._pendingIndex !== null) return this._pendingIndex.get(b4a.toString(key, 'hex')) || 0
  ...
  async *listPendingPromotions({ activeRequests } = {}) {
    ...
    for (const [hex, weight] of index) {
      yield { key: b4a.from(hex, 'hex'), weight }
    }
  }
