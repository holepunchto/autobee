...
module.exports = class Systembee {
  ...
  async _flushUpdate(upd, oplog) {
    const [writer, grants, prunes] = await Promise.all([
      this._updateWriter(upd, oplog),
      this._updateGrants(upd),
      this._pruneGrants(upd)
    ])

    return { writer, grants, prunes }
  }
