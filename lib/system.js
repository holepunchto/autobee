...
module.exports = class Systembee {
  ...
  async commonAncestor(head) {
    ...
    // at migration point: only return usable ancestor
    // if both sides migrated from the same legacy node
    if (topo.isLegacyNode(a.value) || topo.isLegacyNode(b.value)) {
      return legacyAncestor(hist1, a, hist2, b)
    }

    let l = getInfo(a.value)
    let r = getInfo(b.value)

    // no decodable info at all: treat as the legacy boundary
    if (!l || !r) return legacyAncestor(hist1, a, hist2, b)
    ...
    while (!a.done || !b.done) {
      ...
      if (l.flushes >= r.flushes) {
        ...
        a = await hist1.next()
        if (a.value) {
          if (topo.isLegacyNode(a.value)) return legacyAncestor(hist1, a, hist2, b)
          l = getInfo(a.value)
          if (!l) return legacyAncestor(hist1, a, hist2, b)
        }
      }
      ...
      if (r.flushes >= l.flushes) {
        ...
        b = await hist2.next()
        if (b.value) {
          if (topo.isLegacyNode(b.value)) return legacyAncestor(hist1, a, hist2, b)
          r = getInfo(b.value)
          if (!r) return legacyAncestor(hist1, a, hist2, b)
        }
      }
    }
...
async function firstLegacyNode(it, cur) {
  while (cur.value) {
    if (topo.isLegacyNode(cur.value)) return cur.value
    if (cur.done) break
    cur = await it.next()
  }
  return null
...
async function legacyAncestor(hist1, a, hist2, b) {
  const l = await firstLegacyNode(hist1, a)
  const r = await firstLegacyNode(hist2, b)

  if (!l || !r || !sameHead(l.head, r.head)) return null

  const info = getLegacyInfo(l)
  if (!info) return null

  return {
    flushes: info.flushes,
    system: l.head,
    view: info.view || EMPTY_HEAD
  }
}
