...
module.exports = class Systembee {
  ...
  async _loadPendingArrays() {
    if (this._pendingArrays !== null) return this._pendingArrays

    const arrays = new Map()
    for await (const data of this.bee.createReadStream(encoding.encodeSystemPendingRange())) {
      arrays.set(
  ...
  async pendingAtWeight(weight) {
    const arrays = await this._loadPendingArrays()
    return arrays.get(weight) || []
  }
