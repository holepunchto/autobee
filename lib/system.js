...
module.exports = class Systembee {
  ...
  async reset({ timeout } = {}) {
    ...
    this.members = info ? info.members || 0 : 0
    this.updates.clear()
    this.writers.clear()
    ...
    // records from before the tally carry no count
    if (info && this.members === 0) this.members = await this._countMembers()
  }

  async _countMembers() {
    let n = 0
    for await (const w of this.list()) {
      if (!w.isRemoved && !w.isAnchor) n++
    }
    return n
  }
  ...
  addNode(node) {
    for (let i = 0; i < this.heads.length; i++) {
      ...
      if (topo.isLinking(node, h)) {
        ...
        if (this.auto !== null) this.auto._acks.settle(node, h)
      }
    }
  ...
  async _updateWriter(upd, oplog) {
    ...
    const wasMember = v !== null && !v.isRemoved && !v.isAnchor
    const isMember = !record.isRemoved && !record.isAnchor
    ...
    return {
      ...
      members: (isMember ? 1 : 0) - (wasMember ? 1 : 0)
    }
  }
  ...
  async flush(batch, bee) {
    ...
    for (const { key, value, add, remove, members } of results) {
      w.tryPut(key, value)
    ...
    if (this.members < 0) this.members = 0
