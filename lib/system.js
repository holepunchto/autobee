...
module.exports = class Systembee {
  ...
  async addWriter(key, { length = 0, weight = 1, isGenesis = false } = {}) {
    ...
    if (weight <= current) {
      ...
      this.update(key, length, -1, -1, false, true, false, false, false, -1)
      return
    }

    this.update(key, length, -1, weight, isGenesis, true, false, false, false, -1)
  }
  ...
  async ackWriter(key, { length = 0 } = {}) {
    ...
    this.update(key, length, -1, -1, false, false, false, true, false, -1)
  }
  ...
  async removeWriter(key, { length = 0 } = {}) {
    ...
    this.update(key, length, -1, 0, false, false, true, false, false, -1)
  }

  addAnchor(key, { length = 1 } = {}) {
    this.update(key, length, -1, -1, false, true, false, false, true, -1)
  }
