...
module.exports = class Systembee {
  constructor(auto, store, opts = {}) {
    ...
    // writers explicitly added/acked since the last flush - the acceptance
    // signal for optimistic batches (addNode also bumps length, so length
    // alone cannot tell an accepted optimistic writer from a declined one)
    this.acked = new Set()
  ...
  async addWriter(
    ...
    this.acked.add(b4a.toString(key, 'hex'))
  ...
  isAcked(key) {
    return this.acked.has(b4a.toString(key, 'hex'))
  }
