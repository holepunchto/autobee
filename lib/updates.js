...
module.exports = class UpdateChanges {
  ...
  static from(shared, current) {
    if (
      shared.flushes === current.flushes &&
      shared.system.length === current.system.length &&
      (shared.system.key === current.system.key ||
        (shared.system.key &&
          current.system.key &&
          b4a.equals(shared.system.key, current.system.key)))
    ) {
      return null
    }
