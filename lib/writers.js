...
const { trace, traceVerbose, coreState } = require('./debug-log.js')
  ...
  *external() {
  ...
  async wakeup(key, length) {
    ...
    if (w === this.localWriter) {
      traceVerbose(this, 'wakeup: ignoring hint for our own writer', { key, length })
      return
    }
    ...
    if (!w) {
      ...
      if (!w) {
        trace(this, 'wakeup: writer could not be opened', { key, length })
        return
      }
      added = true
    }
    ...
    traceVerbose(this, 'wakeup: writer hinted', {
      key,
      length,
      hinted: w.hintedLength,
      coreLength: w.core.length,
      added,
      advanced,
      bumping: added || advanced
    })
  ...
  async flushLocal(views) {
    ...
    trace(this, 'local: flushing appends', {
      nodes: w.processed,
      system: views.system,
      view: views.view,
      flushes: views.flushes
    })
  ...
  async refresh() {
    ...
    for (const writer of this.active.values()) {
      if (writer.isFrozen && writer !== this.localWriter) {
        trace(this, 'sync: dropping frozen writer', { key: writer.core.key })
  ...
  async _add(key, id) {
    ...
    trace(this, 'sync: opened writer', { key, core: coreState(core) })
  ...
  async remove(key) {
    ...
    if (w !== this.localWriter) {
      ...
      if (length < w.core.length || length < w.hintedLength) {
        trace(this, 'sync: removed writer still has nodes to apply', {
          key,
          applied: length,
          coreLength: w.core.length,
          hinted: w.hintedLength
    ...
    trace(this, 'sync: closing removed writer', { key })
  ...
  _bootstrap() {
    ...
    w.isHead = true // the root of the autobase, never gc'd
    w.attach()
    w.couple()
  }
}
...
class Writer {
  ...
  async _next() {
    ...
    if (length < this.core.length) {
      traceVerbose(this, 'sync: prefetching writer blocks', {
        key: this.core.key,
        start: length,
        end: length + WRITER_PREFETCH,
        core: coreState(this.core)
      })
    }
    ...
    while (length < this.core.length) {
      ...
      if (!(await this.core.has(length))) {
        // peers is remoteLength/remoteContiguousLength per connected peer - an
        // empty list, or one where nobody reaches `need`, means nobody we are
        // talking to can serve this block
        trace(this, 'sync: blocked, writer block not downloaded', {
          key: this.core.key,
          need: length,
          hinted: this.hintedLength,
          core: coreState(this.core),
          servable: this.core.peers.some((p) => p.remoteContiguousLength > length)
        })
        this.removePending()
        return null
      }
      ...
      try {
        ...
        trace(this, 'sync: writer oplog undecodable, freezing', {
          key: this.core.key,
          seq: length - 1,
          message: err.message
        })
      ...
      if (!(await this._isLinked(node))) {
        trace(this, 'sync: blocked, node links not applied yet', {
          key: this.core.key,
          length: node.length,
          links: node.links
        })
