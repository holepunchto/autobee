...
class ActiveWriters {
  ...
  *external() {
  ...
  async flushLocal(views) {
    ...
    const trusted = await this.auto.trusted.mostRecentTrusted(this.auto._workingView.view)

    return w.flush(views, queued, trusted)
  }
...
class Writer {
  ...
  flush(views, queued, trusted) {
    if (this.pending === null || this.processed === 0) return Promise.resolve()

    const head = this.pending[this.processed - 1]
    head.views = views

    head.trusted = trusted
      ? [{ key: trusted.key, length: trusted.length, flushes: views.flushes }]
      : null
