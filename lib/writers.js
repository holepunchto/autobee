...
class Writer {
  ...
  async _next() {
    ...
    while (length < this.core.length) {
      ...
      // a decreasing claim is an order-independent violation (every peer
      // evaluates it identically), so freezing is safe - unlike an unbacked
      // claim, which is merely ignored at resolution
      if (node.claim) {
        if (node.claim.weight < this.claimFloor) {
          this.isFrozen = true
          this.removePending()
          return null
        }
        this.claimFloor = node.claim.weight
      }
