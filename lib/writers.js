...
class ActiveWriters {
  ...
  *external() {
  ...
  async wakeup(key, length) {
    ...
    // a hint can already sit at this length with the node still unapplied (boot probe, a hint
    // applied mid-pass while the block was in flight), so gate on what has been applied too
    const info = await this.auto.system.get(key, { unflushed: true })
    const behind = length > (info ? info.length : 0)

    if (added || advanced || behind) this.bump()
  }
