...
class ActiveWriters {
  ...
  *external() {
  ...
  async toArray(limit = Infinity) {
    const reservoir = []
    let count = 0

    // Get random selection (or all) active writers
    for (const w of this.active.values()) {
      ...
      if (reservoir.length < limit) {
        ...
        if (j < limit) reservoir[j] = w
      }
    }

    // Get the writer views from oplogs
    const results = await Promise.all(reservoir.map((w) => w.views()))

    // two views per writer
    const views = []
...
class Writer {
  ...
  async views() {
    ...
    if (!latest || !latest.views) return null
    return [latest.views.system, latest.views.view]
  }
