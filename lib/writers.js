    ...
    while (length < this.core.length) {
      ...
      for (const att of node.attestations || []) {
        ...
        const seen = this.writers.witnesses.some(
          (w) => w.length === node.length && w.attestation.weight === att.weight && b4a.equals(w.key, node.key)
        )
        if (seen) continue
        ...
        this.writers.witnesses.push({ key: node.key, length: node.length, attestation: att, manifest })
      }
