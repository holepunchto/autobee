...
// race sentinel for claim verification: already-settled verdicts win the
...
class Writer {
  ...
  async _next() {
    ...
    while (length < this.core.length) {
      ...
      // wait for claim verification: may need remote reads, so
      // park this writer instead of blocking the drain loop
      if (node.claim && b.start === 0) {
        try {
          if (!(await this._verifyClaim(node))) {
            this.removePending()
            return null
          }
        } catch (err) {
          ...
          // claim violation is provable misbehaviour
          if (err.code === 'CLAIM_VERIFICATION_FAILED') this.isFrozen = true
          this.removePending()
          return null
        }
      }
  ...
  async _verifyClaim(node) {
    if (this.verifying === null || this.verifyingLength !== node.length) {
      this.verifying = this.writers.auto._verifyClaim(node, this.activeRequests)
    ...
    // failed claim is an order-independent violation
    if (!state) {
      const err = new Error('Claim verification failed')
      err.code = 'CLAIM_VERIFICATION_FAILED'
      throw err
    }
