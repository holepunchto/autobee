...
class ActiveWriters {
  ...
  *external() {
  ...
  // returns every batch that can be applied right now: the ones whose links
  // are in system, plus the ones that were only waiting on those (found through
  // the triggers), and so on until nothing new resolves.
  //
  // dependency order, not sort order - run them through topo.linearize before
  // applying. weights are left at -1. settled is false when a not-yet-added
  // writer had a batch, applying this round may add it
  async nextBatches({ local = false } = {}) {
    const batches = []

    // writer id -> length the collected batches take that writer to, treated as
    // part of system for link checks in this round
    const resolved = new Map()
    const rejected = []

    let queue = new Set(this.pending)
    ...
    while (queue.size > 0) {
      ...
      for (const w of queue) {
        ...
        const batch = await w.next(resolved)
        if (batch === null) continue

        // a batch we won't apply must not resolve anything downstream
        if (!this._accepts(w, batch)) {
          // not added yet is the only rejection applying this round can fix
          if (!w.isAdded && !batch[0].optimistic) rejected.push(w)
          else w.removePending()
          continue
        }

        const tail = batch[batch.length - 1]

        batches.push({ writer: w, batch })
        resolved.set(w.id, tail.length)

        // more behind this? not for unflushed local nodes, their writable check
        // needs this round applied
        if (!w.appending) next.add(w)

        // anyone parked on this writer up to tail is unblocked. also keep them
        // pending, their system state (isAdded etc) needs this round applied
        for (const dep of this.triggers.collect(w.id, tail.length)) {
          dep.waiting = null
          if (dep.isAttached) dep.addPending()
          next.add(dep)
        }
      }
    ...
    // only drop rejected writers when nothing is going to change
    if (batches.length === 0) {
      for (const w of rejected) w.removePending()
    }

    return { batches, settled: rejected.length === 0 }
  }

  _accepts(w, batch) {
    if (this.auto.optimistic && batch[0].optimistic) return true
    if (w.isAdded) return true
    return w.isRemoved && w.hasReferrals()
  }
...
class Writer {
  ...
  isSystemHead() {
    if (this.core.length === 0) return false

    if (this.system.heads.length === 0) {
      return b4a.equals(this.core.key, this.writers.auto.key)
    }

    return this.system.isHead(this.core.key)
  }
  ...
  hint(length) {
    if (length > this.hintedLength) {
      this.hintedLength = length
      this.core.setActive(true)
    }
  }

  _onchange() {
    this.hint(this.core.length)
  ...
  async reset() {
    ...
    if (length < this.core.length || length < this.hintedLength || this.appending) {
      this.addPending()
    } else if (this !== this.writers.localWriter && !this.isHead) {
  ...
  async update() {
    ...
    const isHead = this.isSystemHead()
    ...
    const active = length < this.core.length || length < this.hintedLength

    if (this.isHead || active) {
      this.core.setActive(true)
    } else {
  ...
  // resolved (optional): id -> length map of batches the caller has collected
  // but not applied yet, links to those count as satisfied (see nextBatches)
  next(resolved = null) {
    if (this.isFrozen) return null
    return this._next(resolved)
  }

  async _next(resolved = null) {
    ...
    // continue past what nextBatches already took from us this round
    if (resolved !== null && resolved.has(this.id)) {
      length = Math.max(length, resolved.get(this.id))
    }
    ...
    while (length < this.core.length) {
      ...
      if (!(await this._isLinked(node, resolved))) {
        this.removePending()
        return null
      }
    ...
    if (this.core.writable && this.pending !== null) {
      ...
      for (let i = start; i < end; i++) {
        ...
        if (!(await this._isLinked(node, resolved))) break
  ...
  async _isLinked(node, resolved = null) {
    ...
    for (let i = 0; i < links.length; i++) {
      ...
      // satisfied by a batch collected earlier in this nextBatches round
      if (resolved !== null) {
        const len = resolved.get(b4a.toString(link.key, 'hex'))
        if (len !== undefined && len >= link.length) {
          promises[i] = true
          continue
        }
      }
    ...
    for (let i = 0; i < links.length; i++) {
      ...
      // one trigger is enough to get us rechecked, the recheck parks us on the
      // next missing link. but wake up every dependency so they all get fetched
      if (linked) {
  ...
  async flush(views, trusted) {
    ...
    this.writers.auto.emit('appending', this.pending)
