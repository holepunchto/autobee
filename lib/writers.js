...
class ActiveWriters {
  ...
  *external() {
  ...
  // applying. weights are left at -1.
  async nextBatches({ local = false } = {}) {
    ...
    while (queue.size > 0) {
      ...
      for (const w of queue) {
        ...
        // anyone parked on this writer up to tail is unblocked. links resolve
        // against this round, but system state (isAdded etc) does not until
        // applied - so also queue them as pending for the next bump. the writer
        // itself gets its next batch on the next bump too, same reason
    ...
    // a rejection may be undone by what we apply this round (ie the addWriter
    // for it), so only drop rejected writers when nothing is going to change
    if (batches.length === 0) {
