...
// how long the picker waits for an announced-but-missing block before applying around it
const MAX_DOWNLOAD_WAIT = 1000
...
class ActiveWriters {
  ...
  async close() {
    if (this._downloadTimer !== null) clearTimeout(this._downloadTimer)
    this._downloadTimer = null
  ...
  *external() {
  ...
  async nextPendingNode({ local = false } = {}) {
    ...
    // a peer announced blocks we are still fetching - wait for them (bounded) so the
    // merge sees them, instead of applying a head they may sort before and rebasing
    if (!local && this._waitForDownloads()) return null
  ...
  _waitForDownloads() {
    ...
    for (const w of this._downloading) {
      const left = w.missingSince + MAX_DOWNLOAD_WAIT - now
      if (left > wait) wait = left
    }

    if (wait === 0) return false

    // nothing else may bump us if the block never lands, so time out the wait
...
class Writer {
  ...
  close() {
    ...
    this.writers._downloading.delete(this)
    this.removePending()
  ...
  async _next() {
    ...
    while (length < this.core.length) {
      ...
      if (!(await this.core.has(length))) {
        if (this.missingSince === 0) this.missingSince = Date.now()
        this.writers._downloading.add(this)
        this.removePending()
        return null
      }
