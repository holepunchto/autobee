...
class ActiveWriters {
  ...
  *external() {
  ...
  async _add(key, id) {
    ...
    const group =
      local || !this.auto.wakeupCapability ? null : this.auto.wakeupCapability.discoveryKey

    const core = this.auto.store.get({ key, encryption, group })

    await core.ready()
    if (group) await core.setGroup(group)
