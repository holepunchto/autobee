...
class ActiveWriters {
  ...
  *external() {
  ...
  async _add(key, id) {
    ...
    if (!local && this.auto.wakeupCapability) {
      await core.setGroup(this.auto.wakeupCapability.discoveryKey)
    }
...
class Writer {
  ...
  async _next() {
    ...
    while (length < this.core.length) {
      ...
      for (const trusted of node.trusted) this.writers.auto._onTrustedHead(trusted)
  ...
  flush(views, queued) {
    ...
    const tip = this.writers.auto.trusted.tip
    head.trusted = tip ? [tip] : null
