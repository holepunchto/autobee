...
class Writer {
  constructor(writers, system, local, core, id) {
    ...
    this.witnessPointer = -1
    this._witnessPointerLoaded = false
    this.waiting = null
    this.download = null
    this.hintedLength = 0
  ...
  async _isLinked(node) {
    ...
    if (node.witness && node.witness.data) {
      links = links.slice()
      links.push({ key: node.witness.data.link.key, length: node.witness.data.link.length })
    }
  ...
  async loadWitnessPointer() {
    if (this._witnessPointerLoaded) return this.witnessPointer
    this._witnessPointerLoaded = true

    const length = this.core.length
    if (length === 0) return this.witnessPointer

    const oplog = encoding.decodeOplog(await this.core.get(length - 1))
    const w = oplog.witness
    if (!w) return this.witnessPointer

    const queued = this.appendLength - 1 - length
    this.witnessPointer = (w.data ? 0 : w.pointer) + queued
    return this.witnessPointer
  }
  ...
  append(value, timestamp, batch, links, optimistic, witness, approvals = null) {
    ...
    if (witness && witness.data) this.witnessPointer = 0
    else if (this.witnessPointer >= 0) this.witnessPointer++
