...
const { signAttestation, resolveWeight } = require('./witness.js')
...
class Writer {
  ...
  async _next() {
    ...
    while (length < this.core.length) {
      ...
      batch.push(node)
      if (b.end === 0) {
        if (b.version <= LEGACY_OPLOG_VERSION) {
          upsertLegacyBatchPointers(batch)
        }
        return batch
      }
  ...
  async flush(views, queued, trusted) {
...
exports.upsertLegacyBatchPointers = upsertLegacyBatchPointers
...
function upsertLegacyBatchPointers(batch) {
  for (let i = 1; i < batch.length; i++) {
    batch[i].batch.start = i
  }
}
