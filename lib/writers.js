...
class Writer {
  ...
  async views() {
    ...
    // signedLength for autobase compat (end is the signed length of the view)
    return [
      { key: latest.views.system.key, length: latest.views.system.end, signedLength: latest.views.system.end },
      { key: latest.views.view.key, length: latest.views.view.end, signedLength: latest.views.view.end }
    ]
  }
  ...
  async next() {
    ...
    while (length < this.core.length) {
      ...
      if (this.writable) node.optimsitic = false
