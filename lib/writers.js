  ...
  *external() {
  ...
  views(limit = Infinity) {
    ...
    for (const w of this.active.values()) {
      ...
      views.push({ key: w.core.key, length: w.core.length })
    }

    if (limit >= views.length) return views

    // shuffle views to get a random sample
    for (let i = 0; i < limit; i++) {
      const j = i + Math.floor(Math.random() * (views.length - i))
      const tmp = views[i]
      views[i] = views[j]
      views[j] = tmp
    }
