...
const { signAttestation } = require('./witness.js')
...
class ActiveWriters {
  ...
  *external() {
  ...
  async rotateLocalWriter(core) {
    ...
    // old local key witnesses are useless
    this.witnesses = []
  ...
  flushLocal(views) {
    ...
    if (w.pending === null || w.processed === 0) return Promise.resolve()

    const queued = this.attestations
    this.attestations = []

    return w.flush(views, queued)
  }
  ...
  attest(witnessed) {
    const local = this.localWriter

    for (const { key, weight } of witnessed) {
      if (b4a.equals(key, local.core.key)) continue // never attest about myself
      if (this.attestations.some((a) => a.weight === weight && b4a.equals(a.key, key))) continue

      this.attestations.push({ key, weight })
    }
  }
}
...
class Writer {
  constructor(writers, system, local, core, id) {
    ...
    // the local writer also listens for downloads so a replicated own tail reschedules it
    if (!local) this.core.on('append', this._onchangeBound)
    this.core.on('download', this._onchangeBound)
  ...
  async _next() {
    ...
    while (length < this.core.length) {
      ...
      for (const att of node.attestations || []) {
        if (!b4a.equals(att.key, this.writers.localWriter.core.key)) continue

        const seen = this.writers.witnesses.some(
          (w) =>
            w.length === node.length &&
            w.attestation.weight === att.weight &&
            b4a.equals(w.key, node.key)
        )
        if (seen) continue

        const manifest = this.core.getManifest({ raw: true })
        if (!manifest) continue

        this.writers.witnesses.push({
          key: node.key,
          length: node.length,
          attestation: att,
          manifest
  ...
  async _isLinked(node) {
    ...
    if (node.witness) {
      links = links.slice()
      links.push({ key: node.witness.backer.key, length: node.witness.backer.length })
    }
  ...
  append(value, timestamp, batch, links, optimistic, witness) {
    if (this.pending === null) {
      // an adopted own tail moves the append position forward, an in-flight flush never back
      if (this.core.length >= this.appendLength) this.appendLength = this.core.length + 1
      this.pending = []
    }
  ...
  flush(views, queued) {
    ...
    if (queued && queued.length) {
      const attestations = []
      for (const { key, weight } of queued) {
        const signature = signAttestation(
          this.core.keyPair.secretKey,
          this.core.key,
          key,
          head.length,
          weight
        )
        attestations.push({ key, weight, signature })
      }
      head.attestations = attestations
    }
...
function createNode(core, length, weight, oplog) {
  return {
    ...
    attestations: oplog.attestations || null,
    batch: oplog.batch,
    views: oplog.views,
