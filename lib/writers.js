...
class ActiveWriters {
  ...
  *external() {
  ...
  async add(key) {
    if (b4a.equals(key, this.auto.local.key)) {
      await this.updateLocalState()
      // If local writer has unindexed data, add to pending so it gets processed
      if (this.writable && this.localWriter.core.length > 0) {
