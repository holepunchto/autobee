...
class Writer {
  ...
  async next() {
    ...
    if (this.core.writable && this.pending !== null) {
      let start = this.pendingLength
      this.pendingLength = this.pending.length

      for (let i = start; i < this.pendingLength; i++) {
      ...
      return this.pending.slice(start, this.pendingLength)
    }
  ...
  clear() {
    if (this.pending && this.pendingLength !== this.pending.length) {
      throw new Error('Clearing unflushed nodes')
    }
