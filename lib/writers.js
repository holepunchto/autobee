...
// race sentinel for witness verification: already-settled verdicts win the
// race, pending ones park the writer instead of blocking the drain loop
const VERIFY_PENDING = Symbol('verify pending')
const VERIFY_PENDING_PROMISE = Promise.resolve(VERIFY_PENDING)
...
class ActiveWriters {
  ...
  *external() {
  ...
  appendLocal(value, timestamp, batch, links, optimistic, witness) {
    return this.localWriter.append(value, timestamp, batch, links, optimistic, witness)
  }
...
class Writer {
  ...
  async reset() {
    ...
    // don't drop any local nodes left in buffer
    if (length < this.core.length || this.appending) {
      this.addPending()
  ...
  close() {
    ...
    Hypercore.destroyRequests(this.activeRequests, null)
  ...
  // witnesses ride the first node of a batch, so resolve the batch start
  async latestWitness() {
    if (this.pending) {
      for (let i = this.pending.length - 1; i >= 0; i--) {
        if (this.pending[i].witness) return this.pending[i].witness
      }
    }
    ...
    if (head.witness) return head.witness
  ...
  async _next() {
    ...
    while (length < this.core.length) {
      ...
      if (b.start === 0) {
        const links = node.witness ? node.links.concat([node.witness.backer]) : node.links
        for (const link of links) {
          try {
            if (!(await this._checkLinkTimestamp(node, link))) {
              this.removePending()
              return null
            }
          } catch (err) {
            ...
            if (err.code === 'TIMESTAMP_VIOLATION') this.isFrozen = true
            this.removePending()
            return null
          }
        }
      ...
      // wait for witness verification: may need remote reads, so
      // park this writer instead of blocking the drain loop
      if (node.witness && b.start === 0) {
        try {
          if (!(await this._verifyWitness(node))) {
            this.removePending()
            return null
          }
        } catch (err) {
          ...
          // witness violation is provable misbehaviour
          if (err.code === 'WITNESS_VERIFICATION_FAILED') this.isFrozen = true
          this.removePending()
          return null
        }
      }
  ...
  async _verifyWitness(node) {
    if (this.verifying === null || this.verifyingLength !== node.length) {
      this.verifying = this.writers.auto._verifyWitness(node, this.activeRequests)
      this.verifyingLength = node.length
    }

    const verdict = this.verifying

    let state
    try {
      state = await Promise.race([verdict, VERIFY_PENDING_PROMISE])
    } catch (err) {
      // consumed the rejection - a retry starts a fresh verification
      this.verifying = null
      throw err
    }

    if (state === VERIFY_PENDING) {
      // run verification in bg
      verdict.then(
        () => {
          if (this.isClosed) return
          this.addPending()
          this.writers.auto.bumpSoon()
        },
        // rejection (incl. cancellation on close): retry on the next
        // natural bump, deliberately without scheduling one (no error
        // hot loop) - the retry pass consumes the rejection above
        () => {
          if (this.isClosed) return
          this.addPending()
        }
    ...
    // failed witness is an order-independent violation
    if (!state) {
      const err = new Error('Witness verification failed')
      err.code = 'WITNESS_VERIFICATION_FAILED'
      throw err
    }
  ...
  async _checkLinkTimestamp(node, link) {
    if (link.length === 0) return true

    const core = this.writers.auto.openCore(link.key)
    const seq = link.length - 1

    let pending = true
    try {
      if (await core.has(seq)) {
        pending = false

        const oplog = encoding.decodeOplog(await core.get(seq))
        if (oplog.timestamp <= node.timestamp) return true

        const err = new Error('Timestamp violation')
        err.code = 'TIMESTAMP_VIOLATION'
        throw err
      } else {
        // keep the session alive until the fetch settles, then retry
        core
          .get(seq, { activeRequests: this.activeRequests })
          .then(() => {
            if (this.isClosed) return
            this.addPending()
            this.writers.auto.bumpSoon()
  ...
  async _isLinked(node) {
    // the backer gates like a link: present-or-wait
    let links = node.links
    if (node.witness) {
      links = links.slice()
      links.push(node.witness.backer)
    }
  ...
  append(value, timestamp, batch, links, optimistic, witness) {
  ...
  flush(views) {
    if (this.pending === null || this.processed === 0) return Promise.resolve()
    const buffers = []
    this.pending[this.processed - 1].views = views
