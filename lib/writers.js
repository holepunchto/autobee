...
class ActiveWriters {
  ...
  *external() {
  ...
  // SKETCH: returns every batch that can be applied right now: the ones whose
  // links are in system, plus the ones that were only waiting on those (found
  // through the triggers), and so on until nothing new resolves.
  //
  // dependency order, not sort order - prepareBatch / topo.sort still places
  // each one. weights are left at -1 like nextPendingNode.
  async nextBatches() {
    const batches = []
    ...
    while (queue.size > 0) {
      ...
      for (const w of queue) {
        ...
        // same writer may have more, and anyone parked on it up to tail is unblocked
        next.add(w)
...
class Writer {
  ...
  // but not applied yet, treated as if they were in system (see nextBatches)
  next(resolved = null) {
