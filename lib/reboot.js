...
module.exports = class Reboot {
  ...
  async _run() {
    console.log('booting from', this.head, this.trace)
    await this.system.boot(this.head)

    console.log('system version', this.system.version)

    // a legacy system needs migrating rather than a plain fast-forward
    if (this.system.version < AUTOBEE_VERSION) return this._migrate()
