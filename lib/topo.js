...
const { createNode, finalizeNodeBatch, isUserOp } = require('./writers.js')
const asserts = require('./asserts.js')
const { LEGACY_OPLOG_VERSION } = require('./constants.js')

const REPLAY_MIN_CHUNK = 16
...
exports.replayLast = replayLast
exports.nodeFilter = nodeFilter
exports.isAnyOp = isAnyOp
...
function nodeFilter({ filter = isUserOp } = {}) {
  return filter || isAnyOp
}

async function replayLast(auto, n, opts) {
  n = Math.floor(n)
  if (!(n > 0)) return []

  const keep = nodeFilter(opts)
  const rounds = []
  ...
  for await (const entry of oplogIterator(auto.system.bee)) {
    ...
    for (const ref of entry.refs) refs.push(ref)
    if (refs.length < target) continue

    found += await inflateRound()
    if (found >= n) break

    target = Math.max(n - found, REPLAY_MIN_CHUNK)
  }

  await inflateRound()

  return rounds.reverse().flat().slice(-n)

  async function inflateRound() {
    if (!refs.length) return 0

    const batch = refs.reverse()
    refs = []

    const inflated = await inflate(auto, batch, { wait: false })
    const nodes = inflated.flatMap((b) => (b === null ? [null] : b.batch))
    const kept = nodes.filter((node) => node === null || keep(node))
