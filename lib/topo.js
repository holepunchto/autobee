...
function getLocalBatch(undo, writer, length, weight) {
  if (length > writer.core.length) {
    ...
    // A freshly-appended pending node has batch === null (appendLocal stores
    // null; the span is computed lazily in Writer.next()/flush()). If a sort
    // references this local node before that happens, fall back to the same
    // whole-pending-array span those methods assign rather than throwing.
    const headBatch = head.batch || { start: i, end: writer.pending.length - 1 - i }
