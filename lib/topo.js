...
async function sort(auto, batch) {
  ...
  for await (const data of ch) {
    ...
    if (result === null) {
      // system-only update (e.g. writer removal) — no oplog to reorder,
      // but still check timestamp to know if we've walked far enough back
      const info = getInfo(data)
      if (info && node.timestamp > info.timestamp) break
      continue
    }
...
function getInfo(data) {
  for (let i = 0; i < data.batch.length; i++) {
    ...
    for (let j = 0; j < keys.length; j++) {
      const k = keys[j]
      if (k.key[0] === 0) return encoding.decodeSystemInfo(k.value)
    }
  }
  return null
}
