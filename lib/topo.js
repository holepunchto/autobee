...
async function sort(auto, batch) {
  ...
  for await (const data of ch) {
    ...
    const isLinked = isLinking(node, next)

    if (next.weight === node.weight && (isLinked || next.timestamp < node.timestamp)) break
...
async function getOplogBatch(undo, core, length, weight) {
  ...
  const head = createNode(core, length, oplog, weight)
  ...
  if (remaining.length) {
    ...
    for (let i = 0; i < blocks.length; i++) {
      ...
      batch.push(createNode(core, i + start + 1, oplog, weight))
    }
  }
