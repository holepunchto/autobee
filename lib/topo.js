...
async function sort(auto, batch) {
  ...
  for await (const data of ch) {
    ...
    const link = {
      ...
      // NOT ref.oplog.ackedWeight - that's frozen as of whenever THIS peer
      // happened to flush this specific historical batch, which can differ
      // from another peer's flush timing relative to when a credit landed.
      // re-derive the writer's current, watermark-protected ackedWeight so
      // every comparison - fresh incoming node or historical reorder-window
      // entry - uses the same converged value
...
async function inflate(auto, reorder) {
  ...
  for (const ref of reorder) {
    ...
    // same reasoning as sort()'s link.weight - re-derive fresh, don't trust
    // the frozen-at-flush-time persisted value
