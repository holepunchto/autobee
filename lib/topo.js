...
exports.getOplogBatch = getOplogBatch // for tests
...
async function getOplogBatch(undo, core, length, weight, index, opts = null) {
  ...
  if (oplog.version <= LEGACY_OPLOG_VERSION) {
    const batch = await getLegacyBatch(core, seq, weight, opts)
...
async function getLegacyBatch(core, seq, weight, opts) {
  const batch = []

  for (let i = seq - 1; i >= 0; i--) {
    const block = await core.get(i, opts)

    // wait: false only - membership is undecidable without the block
    if (block === null) return null

    const oplog = encoding.decodeOplog(block)
    if (oplog.batch.end !== seq - i) break

    batch.push(createNode(core, i + 1, weight, oplog))
  }
