...
async function sort(auto, batch) {
  ...
  for await (const data of ch) {
    ...
    if (skipped > 0) console.log('[topo] skipped', skipped, 'system-only changes')
    skipped = 0

    const { ref, info } = result
    const link = { key: ref.oplog.key, length: ref.oplog.length }

    console.log('[topo] change: oplog', b4a.toString(ref.oplog.key, 'hex').slice(0, 8) + '..', 'len:', ref.oplog.length, 'ts:', info.timestamp, 'linking:', isLinking(node, link), 'newer:', node.timestamp > info.timestamp)

    if (isLinking(node, link) || node.timestamp > info.timestamp) break
    reorder.push(ref)
  }
  if (skipped > 0) console.log('[topo] skipped', skipped, 'system-only changes (end of stream)')
