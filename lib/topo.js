...
function isLinking(node, link, explicit) {
  if (!explicit && node.length >= link.length && b4a.equals(node.key, link.key)) {
    return true
  }
...
function isLinkingAll(node, heads, { explicit = false } = {}) {
  for (const h of heads) {
    if (!isLinking(node, h, explicit)) return false
  }
