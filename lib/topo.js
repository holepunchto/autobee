...
exports.isLegacyFlush = isLegacyNode

// helpers for lib/claims.js: identify a flush block in the changes stream
exports.flushRef = flushRef
exports.isLegacyFlush = isLegacyNode

function flushRef(data) {
...
function addSorted(list, entry) {
  ...
  // referrer/backer are ordering floors: any reorg changing their context
  // rewinds past them and so reapplies this node (makes recompute-on-reapply
  // weight resolution sound)
  if (node.claim) {
    if (node.claim.referrer) clock.add(node.claim.referrer)
    if (node.claim.backer) clock.add(node.claim.backer)
  }
...
async function inflate(auto, reorder) {
  ...
  for (let i = 0; i < reorder.length; i++) {
    ...
    // only seeds the node - reapplication re-resolves from the claim
    ...
    promises.push(getOplogBatch(ref.undo, core, ref.oplog.length, weight, i))
  }
...
async function getOplogBatch(undo, core, length, weight, index) {
  const seq = length - 1
