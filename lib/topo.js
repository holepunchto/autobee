...
function isOrderedBefore(node, oplog) {
  ...
  if (oplog.weight === GENESIS_WEIGHT) return false
...
async function inflate(auto, reorder) {
  ...
  for (const ref of reorder) {
    ...
    if (local && local.id === id) {
      promises.push(getLocalBatch(ref.undo, local, ref.oplog.length, ref.weight))
      continue
    }
    ...
    promises.push(getOplogBatch(ref.undo, core, ref.oplog.length, ref.weight))
  }
  ...
  for (const { keys } of data.batch) {
    for (const k of keys) {
      ...
      if (prefix === 0) {
        ...
        if (result.ref) {
          result.ref.weight = result.info.weight
          return result
        }
      }
