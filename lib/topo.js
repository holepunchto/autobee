...
async function sort(auto, batch) {
  ...
  // an anchor is just a writer whose single node is never applied, so the
  // changes stream cannot surface it - resolve linked anchors and let the
  // anchor node's own links join the causal clock
  await expandAnchorLinks(auto, node)
...
async function getOplogBatch(undo, core, length, weight, index, opts = null) {
  ...
  if (legacy) {
    for (let i = length - 1; i > 0; i--) {
      const block = await core.get(i - 1, opts)
      const oplog = encoding.decodeOplog(block)
...
// a link may point at an anchor: a system-registered writer whose single
// node exists purely to pin another node. the anchor node is never applied,
// so ordering can only see through it by reading the node itself - decrypted
// and decoded exactly like any other writer node
async function expandAnchorLinks(auto, node) {
  if (!node.links || !node.links.length) return

  let expanded = null

  for (const link of node.links) {
    const record = await auto.system.get(link.key)
    if (!record || !record.isAnchor) continue

    const core = auto.openCore(link.key)
    try {
      const block = await core.get(0, { timeout: DEFAULT_OP_TIMEOUT })
      if (block === null) continue

      const oplog = encoding.decodeOplog(block)
      if (expanded === null) expanded = node.links.slice()
      for (const l of oplog.links) expanded.push(l)
    } catch {
      // unresolvable anchor block - keep the raw link, admission gated on it
    } finally {
      await core.close()
    }
  }

  if (expanded !== null) node.links = expanded
}
