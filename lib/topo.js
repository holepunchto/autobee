...
const { createNode, finalizeNodeBatch } = require('./writers.js')
...
// dry-runs the insertion above over a set of batches and returns them in the
// order they settle in. applying them in that order appends every one at the
// tail, so nothing is undone. the fixpoint does not depend on the order the
// batches come in, but each batch must arrive after the batches it links
...
function addSorted(list, entry) {
  ...
  if (node.witness) clock.add(node.witness.link)
...
async function getOplogBatch(undo, core, length, weight, index, readOpts = null) {
  const seq = length - 1

  const block = await core.get(seq, readOpts)
  ...
  // legacy nodes carry the remaining count, so start decodes as 0 on every
  // node - walk back until the previous batch's head (end 0). links do not
  // mark the start: autobase computed heads after adding the previous node,
  // so every node in a legacy batch links its own predecessor.
  // every block inside the batch was read locally when it was applied, so the
  // only block that can be missing is the one before the start (legacy
  // fast-forward never fetched it) - never wait on it, a miss is the boundary
  if (legacy) {
  ...
  for (let i = start; i < end; i++) {
    remaining.push(core.get(i, readOpts))
  }
