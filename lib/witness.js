...
// advisory: a witness through a dead backer is useless
...
async function resolveWeight(auto, node) {
  ...
  if (!backer || backer.isRemoved || backer.length < witness.backer.length) {
    return prev
  }

  // floor is previous weight, cap is between system, backer and witness
  return Math.max(prev, Math.min(witness.weight, currentWeight(backer), limit))
}

// pure function of the node, so every peer agrees
async function verifyWitness(auto, node, activeRequests) {
  const witness = node.witness
  if (!witness || !witness.backer || witness.weight === 0) return false
  if (b4a.equals(witness.backer.key, node.key)) return false

  const witnessed = await witnessedWeight(auto, node, witness.backer, { activeRequests })
  return witnessed === witness.weight
}

// open the backer's block, checkout the system it references, read the
// writer's entry. advisory probes bound with { timeout }
