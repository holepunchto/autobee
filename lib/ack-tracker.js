...
module.exports = class AckTracker {
  ...
  // shortest appends, the rest observe the link and stand down. the window
  // scales with the writer count so the expected gap between draws stays at
  // BACKOFF_TARGET however many writers are racing
  delay(localKey, now, members) {
    const window = Math.max(members, 1) * BACKOFF_TARGET
