...
async function checkMigration(store, local, bootstrap) {
  ...
  const { key, views } = encoding.decodeAutobaseBootRecord(bootRecord)
  ...
  await Promise.all([
    local.setUserData('autobase/boot'),
    local.setUserData('autobase/encryption')
  ])

  // result.system = { key, length: core.length }
}
...
module.exports = async function boot(
  ...
  console.log('migrate', key, keyPair)
  if (key || keyPair) {
    await result.bootstrap.setUserData('referrer', result.key)
    await result.bootstrap.setUserData('autobee/local', result.local.key)
    await result.local.setUserData('referrer', result.key)

    await checkMigration(corestore, result.local, result.bootstrap, result.encryptionKey)
  }
