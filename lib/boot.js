...
module.exports = async function boot(
  ...
  if (key || keyPair) {
    ...
    result.migration = await checkAutobaseMigration(
      corestore,
      result.local,
      result.bootstrap,
      legacyViews
...
async function getWriterBatch(store, head, key, encryptionKey, nodes = []) {
  ...
  // legacy (manifest <= 1) writer cores always take the compat encryption
  // branch in WriterEncryption, which derives a key even when encryptionKey
  // is null - only set a provider when the base was actually encrypted, same
  // as Autobee's own _getEncryptionProvider does for non-migration cores
  if (encryptionKey) {
