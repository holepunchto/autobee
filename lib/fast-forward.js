...
module.exports = class FastForward {
  ...
  static async fromHeads(
    ...
    trace(auto, 'ff: read candidate heads', {
      asked: heads.length,
      withSystem: ops.filter((o) => o !== null).length
    })
  ...
  static async flushHead(auto, head, opts) {
    ...
    try {
      ...
      if (oplog === null) {
        trace(auto, 'ff: head oplog could not be read', {
          head,
          conservative: !!(opts && opts.conservative),
      ...
      if (!oplog.op.views) {
        trace(auto, 'ff: head carries no system info (not an indexer)', {
          head,
          version: oplog.op.version
        })
        return null
      }
