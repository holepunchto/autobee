...
module.exports = class FastForward {
  ...
  static async fromHead(
    ...
    head,
    trusted,
    { force = false, timeout = 0, condition = null } = {}
  ) {
    const conservative = !force && auto._conservativeFF
    const timeoutOpts = timeout ? { timeout } : null

    const oplog = await auto._getOplog(head.key, head.length, {
      conservative,
      ...timeoutOpts
    })
    ...
    const verified = trusted
      ? await auto._getOplog(trusted.key, trusted.length, timeoutOpts)
      : oplog
    ...
    if (condition !== null && !(await FastForward.accepts(auto, verified, condition))) {
      return null
    }
    ...
    const tip = {
      ...
      verified: {
        op: trusted || { key: head.key, length: head.length },
  ...
  static async fromHeads(auto, heads, { force = false, timeout = 0, condition = null } = {}) {
    const promises = []

    for (const head of heads) {
      if (head.length === 0) continue
      promises.push(auto._getOplog(head.key, head.length, timeout ? { timeout } : null))
    }
    ...
    const trust = await Promise.all(
      ops.map((res) => {
        return res === null ? false : auto.trusted.isTrusted(res.key, auto._workingView)
      })
    )
    if (auto.fastForwarding || auto.fastForwardTo) return null
    ...
    if (bestTrusted !== null) {
      return FastForward.fromHead(auto, bestTrusted, null, { force, timeout, condition })
    }

    candidates.sort((a, b) => b.op.views.flushes - a.op.views.flushes)

    for (const res of candidates) {
      const trusted = await FastForward.mostRecentTrusted(auto, res)
      if (trusted === null) continue
      if (auto.fastForwarding || auto.fastForwardTo) return null

      const ff = await FastForward.fromHead(auto, res, trusted, { force, timeout, condition })
  ...
  static async accepts(auto, oplog, condition) {
    const v = oplog.op.views.view
    if (!v) return false

    const view = auto.bee.checkout({ key: v.key, length: v.start + v.length })

    try {
      return !!(await condition(view))
    } finally {
      await view.close()
    }
  }

  static async mostRecentTrusted(auto, head) {
    const v = head.op.views.view
    if (!v) return null

    const view = auto.bee.checkout({ key: v.key, length: v.start + v.length })

    try {
      return (await auto.trusted.mostRecentTrusted(view)) || null
    } finally {
      await view.close()
    }
  }
