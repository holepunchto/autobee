...
module.exports = class FastForward {
  ...
  async _migrate() {
    ...
    for (let hops = 0; hops < MAX_MIGRATE_HOPS; hops++) {
      ...
      const { legacy, autobee } = await this._findNewestSystem(info)
      if (this.destroyed) return null
  ...
  async _findNewestSystem(info) {
    ...
    for (const idx of info.indexers) {
      const core = this.auto.openCore(idx.key)
      this.cores.push(core)
      await core.ready()
      ...
      let oplog = await FastForward.flushHead(
        this.auto,
        { key: idx.key, length },
        { timeout: this.timeout }
      )
      ...
      if (oplog.op.version > LEGACY_OPLOG_VERSION) {
        ...
        const boundary = await this._findLegacyBoundary(core, idx.length, oplog.length - 1)
        if (!boundary || this.destroyed) continue

        oplog = await FastForward.flushHead(
          this.auto,
          { key: idx.key, length: boundary },
          { timeout: this.timeout }
        )
        if (oplog === null || !oplog.op.views) continue
  ...
  async _findLegacyBoundary(core, lo, hi) {
    while (lo < hi) {
      const mid = (lo + hi) >>> 1
