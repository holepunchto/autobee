...
module.exports = class FastForward {
  ...
  static async fromHeads(auto, heads, { timeout = 0 } = {}) {
    const promises = []
    ...
    for (let i = 0; i < ops.length; i++) {
      ...
      if (res.op.views.flushes - auto.system.flushes < MIN_FF_GAP) continue
    ...
    if (bestTrusted !== null) {
      return FastForward.fromHead(auto, bestTrusted, null, { timeout })
    }
    ...
    for (const res of candidates) {
      ...
      const ff = await FastForward.fromHead(auto, res, trusted, { timeout })
