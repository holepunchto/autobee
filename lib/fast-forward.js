...
module.exports = class FastForward {
  ...
  async _migrate() {
    ...
    for (let hops = 0; hops < MAX_MIGRATE_HOPS; hops++) {
      ...
      const next = await this._findNewestSystem(info)
      if (this.destroyed) return null

      if (next !== null && !seen.has(b4a.toString(next, 'hex'))) {
        head = { key: next, length: 0 }
        continue
      }
      ...
      if (this.destroyed || indexerManifests.some((m) => m === null)) return null
  ...
  // read each indexer's newest oplog node - its digest/checkpoint stamp the
  // newest system that indexer knows. take the majority vote
  async _findNewestSystem(info) {
    const tally = new Map()

    for (const idx of info.indexers) {
      let length = 0

      const core = this.auto.store.get(idx.key)
      this.cores.push(core)
      await core.ready()

      length = await coreLength(core, this.timeout)
      if (!length || this.destroyed) continue
      ...
      // an autobee-format stamp means this writer already migrated off the
      // legacy stack - its views point at an autobee system, not a legacy
      // successor, so it is no evidence of a newer legacy generation
      if (oplog.op.version > LEGACY_AUTOBASE_VERSION) continue
