...
class FastForward {
  ...
  static async fromHeads(auto, heads, { force = false, timeout = 0 } = {}) {
    ...
    const ops = await FastForward.v1Candidates(auto, await Promise.all(promises), {
      timeout: timeout || DEFAULT_OP_TIMEOUT
    })
  ...
  static async v1Candidates(auto, ops, { timeout }) {
    const withViews = ops.filter((res) => res !== null && res.op.views)
    const v1 = await Promise.all(withViews.map((res) => FastForward.isV1(auto, res, timeout)))
    return withViews.filter((res, i) => v1[i])
  }

  static async isV1(auto, res, timeout) {
    const { system, view } = res.op.views
    const checks = [system, view].filter(v => !!v).map((range) => {
      const { key, length } = batchToHead(range)
      if (length === 0) return true
      return auto._isV1Core(key, { unknown: false, length, timeout })
    })

    for (const v1 of await Promise.all(checks)) {
      if (!v1) return false
    }
  ...
  static async mostRecentTrusted(auto, head, reference, timeout) {
    const opened = auto.openViewAt(head, { timeout })
  ...
  _startWarmup() {
    ...
    this.warmupView = this.auto.openView(this.system.view, { timeout: this.timeout })
