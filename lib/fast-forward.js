...
module.exports = class FastForward {
  ...
  static async mostRecentTrusted(auto, head, reference, timeout = 0) {
    const opened = auto.openViewAt(head)
    if (opened === null) return null

    try {
      const promise = auto.trusted.mostRecentTrusted(opened.view, reference)
      const trusted = timeout
        ? await new Promise((resolve) => {
            Promise.resolve(promise).then(resolve, () => resolve(null))
