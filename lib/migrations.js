...
const { sortsBefore } = require('./writers.js')
...
// hands out the catchup in sort order rather than legacy INFO order, so it
// does not rebase on itself. only heads whose links are satisfied are
// candidates, and the best of those by sortsBefore goes first. the caller
// resolves weights on the candidates before picking
class CatchupOrder {
  constructor(batches) {
    this.queues = new Map()
    // per catchup writer, the length the system holds so far. a link into a
    // writer outside the map is already in the migrated system - the legacy
    // linearization guarantees it
    ...
    for (const batch of batches) {
      ...
      if (q) q.push(batch)
      else this.queues.set(id, [batch])
    }

    for (const [id, q] of this.queues) this.applied.set(id, q[0][0].length - 1)
  }
  ...
  candidates() {
    ...
    for (const q of this.queues.values()) {
      if (this._isCandidate(q[0][0])) candidates.push(q[0])
    }

    if (candidates.length === 0 && this.queues.size > 0) {
      throw new Error('Migration catchup has unresolvable links')
    }
  ...
  pick(candidates) {
    ...
    for (const batch of candidates) {
      if (best === null || sortsBefore(batch[0], best[0])) best = batch
    }
    return best
  }
  ...
  shift(batch) {
    ...
    assert(q && q[0] === batch, 'Catchup batch shifted out of order')

    q.shift()
    this.applied.set(id, batch[batch.length - 1].length)
    if (!q.length) this.queues.delete(id)
  }

  _isCandidate(node) {
    for (const l of node.links) {
      const length = this.applied.get(b4a.toString(l.key, 'hex'))
      if (length !== undefined && length < l.length) return false
    }
    return true
  }
}
