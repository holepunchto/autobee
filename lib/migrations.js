...
async function checkAutobaseMigration(store, local, bootstrap, legacyViews) {
  ...
  // the core can carry verified-but-undownloaded blocks past the signed head -
  // only systemLength is guaranteed local
  const system = { key, length: systemLength }
  ...
  const catchup = await getCatchupHeads(core, systemLength)
  const nodes = await Promise.all(
    catchup.map((n) => getWriterBatch(store, n, bootstrap.key, encryptionKey))
  )

  const info = await readLegacySystemInfo(core, systemLength)
...
async function getCatchupHeads(core, from) {
