...
module.exports = {
  ...
  resolveLegacyViews,
  inflateLegacyOplog,
  getWriterBatch, // exposed for tests
...
async function checkAutobaseMigration(store, local, bootstrap, legacyViews) {
  ...
  const info = await readLegacySystemInfo(core, core.length)
...
async function readLegacySystemInfo(core, length, opts = null) {
  ...
  const node = decodeBlock(await core.get(length - 1, opts))
...
async function inflateLegacyOplog(buf, core, seq, opts = null) {
  ...
  fetches.push(m.checkpoint.pointer ? core.get(seq - m.checkpoint.system.checkpointer, opts) : buf)
...
async function getWriterBatch(store, head, key, encryptionKey, nodes = []) {
  ...
  // a legacy block only counts how many of its batch follow it, so the head
  // describes no start - walk back while a counter puts its own head at seq
  for (let i = seq - 1; i >= 0; i--) {
    const block = await core.get(i, { wait: false })
    if (!block) break

    const node = encoding.decodeOplog(block)
    if (node.batch.end !== seq - i) break

    batch.unshift({ key: head.key, length: i + 1, from: core, ...node })
  }
