...
exports.Copier = class Copier {
  ...
  get(head) {
    if (this.last === null || !sameHead(this.last.from, head)) return null
    return this.last.to
  }
  ...
  async _append(changes, rewrite) {
    const self = this
    const { bee, config, local, copied, blocks, base } = this
    ...
    for (let i = changes.length - 1; i >= 0; i--) {
      ...
      this.last = { from: change.head, to: { key: local.core.key, length: start + batch.length } }
    }
    ...
    async function previous(tail) {
      ...
      const to = self.get(tail)
      if (to !== null) return { core: 0, seq: to.length - 1 }
...
function sameHead(a, b) {
  return a.length === b.length && b4a.equals(a.key, b.key)
}
